...
// --- SCREENER HISTORY ---
export const getScreenerHistory = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const phq9Result = await query(`SELECT
            ...
            LIMIT 20`, [userId]);
        const gad7Result = await query(`SELECT
                DATE(taken_at) as date,
                total_score,
                severity_label
            FROM gad7_responses
...
export const getSessionStats = async (req, res, next) => {
    try {
        ...
        const conversationsResult = await query('SELECT COUNT(*) as count FROM conversations WHERE user_id = $1', [userId]);
        const journalResult = await query('SELECT COUNT(*) as count FROM journal_entries WHERE user_id = $1', [userId]);
        const thoughtRecordsResult = await query('SELECT COUNT(*) as count FROM thought_records WHERE user_id = $1', [userId]);
        const moodCheckinsResult = await query('SELECT COUNT(*) as count FROM mood_checkins WHERE user_id = $1', [userId]);
        // Get first session date
        const firstSessionResult = await query(`SELECT MIN(created_at) as first_date FROM (
        ...
        const streakResult = await query(`SELECT DISTINCT DATE(created_at) as activity_date FROM (
                SELECT created_at FROM conversations WHERE user_id = $1
                UNION ALL
                SELECT created_at FROM journal_entries WHERE user_id = $1
                UNION ALL
