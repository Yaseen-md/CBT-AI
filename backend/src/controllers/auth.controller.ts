...
const signToken = (payload: { id: string; email: string; name: string; is_admin: boolean; country?: string; has_consented?: boolean }) => {
...
export const register = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = await query(
            'INSERT INTO users (name, email, password_hash) VALUES ($1, $2, $3) RETURNING id, name, email, is_admin, country, has_consented, created_at',
...
export const getMe = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const result = await query(
            'SELECT id, name, email, is_admin, country, has_consented, created_at FROM users WHERE id = $1',
            [req.user!.id]
        );
        const user = result.rows[0];
...
export const provideConsent = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        const userId = req.user!.id;
        
        // Update user's consent in DB
        const result = await query(
            'UPDATE users SET has_consented = true, updated_at = NOW() WHERE id = $1 RETURNING id, name, email, is_admin, country, has_consented',
            [userId]
        );
